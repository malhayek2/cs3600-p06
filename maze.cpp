...
void Cell::Draw(int i, int j)
{
	...
	{
		if (j==0){
			unsigned char red = (unsigned) ((i*365+j*674)%256);
			unsigned char green = (unsigned) ((i*123+j*321)%256);
			unsigned char blue = (unsigned) ((i*456+j*654)%256);
	...
	if (mTop)
	{
		...
		unsigned char green = (unsigned) ((i*251+j*654)%256);
	...
	{
		if (i==0){
			unsigned char red = (unsigned) ((i*789+j*987)%256);
			unsigned char green = (unsigned) ((i*876+j*452)%256);
			unsigned char blue = (unsigned) ((i*258+j*852)%256);
			glColor3ub(red, green, blue);
			glBegin(GL_QUADS);
			glVertex3d(i,j,0);
			glVertex3d(i,j+1,0);
	...
	if (mRight)
	{
		unsigned char red = (unsigned) ((i*1111+j*3223)%256);
		unsigned char green = (unsigned) ((i*4628+j*5643)%256);
		unsigned char blue = (unsigned) ((i*5646+j*22348)%256);
		glColor3ub(red, green, blue);
		glBegin(GL_QUADS);
		glVertex3d(i+1,j,0);
		glVertex3d(i+1,j+1,0);
